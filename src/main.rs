...
impl App {
                    ...
                    match key.code {
                        ...
                        KeyCode::Tab => {
                            self.focus = match self.focus {
                                Focus::NodeList => Focus::Conversation,
                                Focus::Conversation => Focus::Input,
                                Focus::Input => Focus::NodeList,
                                Focus::Unfocused => Focus::NodeList,
                            };
                        }
                        _ => match self.focus {
                                ...
                                KeyCode::Enter => {
                                    if let Some(selected_index) = self.node_list_state.selected() {
                                        ...
                                        if let Some(selected_node) = nodes.get(selected_index) {
                                            self.current_contact = Some((*selected_node).clone());
                                        }
                                    }
                                }
                                _ => {}
    ...
    fn draw(&mut self, frame: &mut Frame) {
        ...
        let paragraph = Paragraph::new(text.clone())
            ...
            .block(
                Block::bordered()
                    ...
                    .border_style(if self.focus == Focus::Conversation {
        ...
        let nodes_list_block = Block::bordered()
            ...
            .border_style(if self.focus == Focus::NodeList {
                Style::default().fg(Color::Yellow)
            } else {
                Style::default()
            });
